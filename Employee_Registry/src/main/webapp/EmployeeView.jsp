...
<%
	...
	if(b!=null && b.equals("del")){
		int id=Integer.parseInt(request.getParameter("erid"));
...
<button align="right"><a href="EmployeeDetails.jsp">Back</a></button>
<br>
<table align="center" border="1" rules="all" cellspacing="10" cellpadding="10">
	<tr><th>Registered ID</th><th>Employee ID</th>
		...
		<th>Company</th><th>Employee InDate</th>
		<th>Employee InTime</th><th>Employee OutDate</th>
		<th>Employee OutTime</th><th colspan="2">Process</th></tr>
<%
	for(Employee i:emp){
%>		
	<tr><td><%=i.getEid() %></td><td><%=i.getErid() %></td>
		<td><%=i.getEname() %></td><td><%=i.getEmobile() %></td>		
		<td><%=i.getEcompany() %></td><td><%=i.geteIndate() %></td>
		<td><%=i.geteIntime() %></td><td><%=i.geteOutdate() %></td>
		<td><%=i.geteOuttime() %></td>
		<td><a href="EmployeeUpdate.jsp?ch=edit&eid=<%=i.getEid() %>">Update</a></td>
		<td><a href="EmployeeView.jsp?ch=del&erid=<%=i.getErid()%>">Remove Employee</a></td>
		</tr>			
<%		
	}
%>
